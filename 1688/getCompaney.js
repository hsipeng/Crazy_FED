...
let getComContact = function(url, CALLBACK){
  let contactUrl = url+'/page/contactinfo.htm?spm=a2615.2177701.0.0.7a3d866frv3vbM'

  getHTML(contactUrl,function(html){
    if(html != null){
      var $ = cheerio.load(html,{decodeEntities:false});
      let $contantDiv = $('.app-contactSmall')
      if($contantDiv.attr('data-view-config') !==undefined){
        let comNameStr = JSON.parse($contantDiv.attr('data-view-config'))
        let comName = comNameStr.companyName
        let comAddr = comNameStr.address
        let contactName = $contantDiv.find('.membername').text()
        let contactUrl =  $contantDiv.find('.membername').attr('href')
        let mobile = $contantDiv.find('.m-mobilephone').attr('data-no')
        
        getPersonDetail(contactUrl,function(person){
          let info ={}
          info['comName'] = comName
          info['comAddr'] = comAddr
          info['contactName'] = contactName
          info['mobile'] = mobile
          info['contactUrl'] = contactUrl
          info['province'] = person.province
          info['city'] = person.city
          info['position'] = person.position
          CALLBACK(info)
        })
      }
...
let getPersonDetail = function(url, CALLBACK){
    getHTML(url,function(html){
      if(html != null){
        let detail ={}
        var $ = cheerio.load(html,{decodeEntities:false});
        let location = $('.arae').text().split('-')
        let position = $('.identity').text()
        detail['province'] = location[0]
        detail['city'] = location[1]
...
// getComs(1)
// getComContact('https://shop1406912011781.1688.com')

// getPersonDetail('https://me.1688.com/*xC-i2FIYOmQuMmQYMmIT.html?spm=a2615.7691479.0.0.60cfa90dHQHm25',function(p){
//   console.log(p)
// })
